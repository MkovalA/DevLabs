...
import { IconClockHour5, IconStar, IconSearch, IconArrowRight } from '@tabler/icons-react';
...
export default function OwnPost() {
  ...
  React.useEffect(() => {
    axiosConfigurated.get("/posts/?page=" + current + "&limit=10" + (sortOrder ? "&sort=date" : '') + (selectStatus ? "&status=" + selectStatus : '' ) +
       (selectCategory ? "&categories=" + [selectCategory] : '' ) + "&own=true").then(async (response) => {
  ...
  return (
      <AppShell.Main>
        ...
        <Stack>
          ...
          return (
          <Card shadow="sm" padding="lg" radius="md" withBorder color='primaryBlue' key={post.id} style={{width: "100%"}}>
            <Group position="apart">
              <Link to={"/posts/" + post.id}><Title order={3}>
                {post.title}
              </Title></Link>
                {post.status === 'active' ? <Badge color="green" variant="filled"> Active </Badge> :
            ...
            <Group mt="sm" spacing="xs">
              ...
              <Text color={theme.colors.yellow[7]}><IconStar size={14} style={{ marginRight: 4, verticalAlign: 'middle' }} />{post.like_count} likes</Text>
            </Group>
            ...
            <Group>
              <Link to={"/post-edit/" + post.id}><Button color={theme.colors.green[5]}>Edit</Button></Link>
              <Link to={"/post-delete/" + post.id}><Button color={theme.colors.red[5]}>Delete</Button></Link>
            </Group>
          </Card>
          );
          })}
